    ...
    async with get_nats_broker() as nats:
        with get_session() as db:
            ...
            for _ in range(20):
                ...
                if actions1 and actions1[0].action.startswith(
            ...
            log_simulation_result(
                simulation_id=simulation.id,
                test_name="bf4-agent-decline-conversation-request",
                ticks=simulation.tick,
                success=declined_conversation,
            )
            assert declined_conversation, (
                "Agent did not decline conversation within 20 ticks"
            )
