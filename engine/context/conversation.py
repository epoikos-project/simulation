...
class ConversationContext(BaseContext):
    def build(self, other_agent: Agent, messages: list[Message]) -> str:
        conversation_context = f"You are currently engaged in a conversation with Agent {other_agent.name} (agent_id: {other_agent.id}). "
...
class PreviousConversationContext(BaseContext):
    def build(self, conversation: Conversation) -> str:
        ...
        context = f"In your last conversation (conversation_id: {conversation.id}) with Agent {other_agent.name} (agent_id: {other_agent.id}), you discussed the following:\n"
...
class OutstandingConversationContext(BaseContext):
    def build(self, conversations: list[Conversation]) -> str:
        ...
        for conversation in conversations:
            context += f"- Conversation (conversation_id: {conversation.id}) with Agent (agent_name: {conversation.agent_b.name}) at Tick {conversation.tick}.\n"
